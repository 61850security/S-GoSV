...
char length_2=0x91;
char resrv1_1=0x00; 		     
char resrv1_2=0x38; /* the length of the hash generated is 32 + extension field length (24 bytes) */                   
char resrv2_1=0xD5;		     
char resrv2_2=0x4B; 
...
char SEQUENCE_length=0x36; //length of the extension 
char Auth_value_tag=0xA4;  // Authentication value tag 0xA4. 
char Auth_length=0x34;

char version_tag=0x80; // version tag 0x80
char version_length=0x01; 
char version_value=0x01;

char time_of_Current_key_tag=0x81;  
char time_of_Current_key_length=0x04;
char time_of_Current_key1=0x5B;
char time_of_Current_key2=0x9C;
char time_of_Current_key3=0xFD;
char time_of_Current_key4=0x67;

char time_of_Next_key_tag=0x82;
char time_of_Next_key_length=0x01;
char time_of_Next_key_value=0x3c;

char keyID_tag=0x84;
char keyID_length=0x04;
char keyID_value1=0x00;
char keyID_value2=0x00;
char keyID_value3=0x00;
char keyID_value4=0x01;
...
int main(int argc, char *argv[])
{
    ...
    unsigned char plainText[192],plainText_str[288];
    ...
    int inx=0,i=0,j=0,x,y,z=0,n,p=0,q=0,temp,temp1;
	...
	//printf("\n Plain Text Data in string format:\n");
        for ( i=0; i<137; i++)
        {
		sprintf( &(plainText_str[i * 2]) , "%02x", PDU[i]);		 
	...
	for( p=0;p<8;p++)
        ...
        /* extension fields*/
        plainText[inx++] = SEQUENCE_tag; // Sec Extention Tag. 1 byte 0x30
	plainText[inx++] = SEQUENCE_length; //length of the extension 
	plainText[inx++] = Auth_value_tag;  // Authentication value tag 0xA4. 
	plainText[inx++] = Auth_length;
	plainText[inx++] = version_tag; // version tag 0x80
	plainText[inx++] = version_length; 
	plainText[inx++] = version_value;
	plainText[inx++] = time_of_Current_key_tag;
	plainText[inx++] = time_of_Current_key_length;
	plainText[inx++] = time_of_Current_key1;
	plainText[inx++] = time_of_Current_key2;
	plainText[inx++] = time_of_Current_key3;
	plainText[inx++] = time_of_Current_key4;
	plainText[inx++] = time_of_Next_key_tag;
	plainText[inx++] = time_of_Next_key_length;
	plainText[inx++] = time_of_Next_key_value;
	plainText[inx++] = keyID_tag;
	plainText[inx++] = keyID_length;
	plainText[inx++] = keyID_value1;
	plainText[inx++] = keyID_value2;
	plainText[inx++] = keyID_value3;
	plainText[inx++] = keyID_value4;
	plainText[inx++] = mac_tag;
	plainText[inx++] = mac_length;

	for ( p=0; p<32; p++)
		plainText[inx++]=hash[p];
	
	
  	temp1= (inx/16);
        printf("\n temp1=%d\n",temp1);
	printf("\n inx=%d\n",inx);
	printf("\n plain text + mac value along with header:\n");
        
        for( p=0;p<inx;p++)
		printf(" %02x",plainText[p]);
   	...
   	for (x=0;x< temp1 ;x++)
        {
        	...
        	for (y=0;y<16;y++)
                {
             		plainText1[y]= plainText[z++];
                        //printf(" %02x",plainText[y]);
                }
		begin = clock();
           	cipher=AESEncryption(plainText1,expandedKey);
		end = clock();
        ...
        else
        {
            ...
            printf("\n-------------------------------------\n\n");
            time_hmac=0;time_encrypt=0;len=0;z=0;inx=0;
        }
...
Comilation of the program: 
     #gcc -o MtE_send MtE_send.c -L /usr/local/lib -lssl -lcrypto
...
Output:
 ...
 PlainText string :
 12

 inx=192

 plain text + mac value along with header:
 61 81 86 80 1a 46 52 45 41 2d 47 6f 53 56 2d 31 20 2f 4c 4c 4e 30 24 47 4f 24 67 63 62 30 31 81 03 00 9c 40 82 18 46 52 45 41 2d 47 6f 53 56 2d 31 20 2f 4c 4c 4e 30 24 47 4f 4f 53 45 31 83 0b 46 52 45 41 2d 47 6f 53 56 2d 31 84 08 38 6e bb f3 42 17 28 0a 85 01 01 86 01 0a 87 01 00 88 01 01 89 01 00 8a 01 08 ab 20 83 01 00 84 03 03 00 00 83 01 00 84 03 03 00 00 83 01 00 84 03 03 00 00 83 01 00 84 03 03 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 01 00